import tensorflow as tf
import tensorflow.keras as keras
import tensorflow_addons as tfa
from tensorflow.keras.datasets import fashion_mnist, mnist
import numpy as np
from utils import show, scale


class N2N(keras.Model):
    def __init__(self):
        super(N2N, self).__init__()

    def build(self, input_shape):
        self.model = keras.Sequential([
            keras.layers.InputLayer(input_shape=input_shape),
            keras.layers.Conv2D(filters=64, kernel_size=3, strides=1, padding="SAME"),
            keras.layers.LeakyReLU(0.2),
        ])
        for _ in range(16):
            self.model.add(keras.layers.Conv2D(filters=64, kernel_size=3, strides=1, padding="SAME"))
            self.model.add(keras.layers.BatchNormalization())
            self.model.add(keras.layers.LeakyReLU(0.2))
        self.model.add(keras.layers.Conv2D(filters=input_shape[-1], kernel_size=3, strides=1, padding="SAME"))
        self.built = True

    def call(self, inputs, training=None, mask=None):
        clean = self.model(inputs)
        return clean


(x_train, _), (x_test, _) = fashion_mnist.load_data()

x_train = scale(x_train)[..., np.newaxis]
x_test = scale(x_test)[..., np.newaxis]

noise_factor = 0.5
x_test_noisy = x_test + noise_factor * tf.random.normal(shape=x_test.shape, stddev=2.0)
x_test_noisy = tf.clip_by_value(x_test_noisy, clip_value_min=-1., clip_value_max=1.)


def get_data(epochs=1000, batch_size=32):
    for _ in range(epochs):
        for i in range(x_train.shape[0] // batch_size):
            start = i * batch_size
            end = min((i + 1) * batch_size, x_train.shape[0])
            noise_image = x_train[start:end] + noise_factor * tf.random.normal(shape=(batch_size, *x_train.shape[1:]), stddev=2.0)
            noise_image = tf.clip_by_value(noise_image, clip_value_min=-1., clip_value_max=1.)
            yield noise_image, noise_image


model = N2N()
model.build(x_train.shape[1:])
model.compile(optimizer=tfa.optimizers.AdamW(learning_rate=1e-4, weight_decay=5e-5),
              loss=keras.losses.MeanSquaredError())
model.fit(get_data(), epochs=10, shuffle=True)
show(x_test_noisy[:16], model(x_test_noisy[:16]))
